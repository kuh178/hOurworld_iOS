...
@interface MTBRequestViewController ()
...
@implementation MTBRequestViewController
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    self.screenName = @"RequestViewController";
}
...
- (UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSLog(@"http://www.hourworld.org/%@", tItem.mProfileImage);
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSLog(@"%f", labelSize.height + height);
...
- (void) downloadContent {
    ...
    NSDictionary *params = @{@"requestType"     :@"Requests,30",
        ...
        constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
              ...
              if ([[responseObject objectForKey:@"success"] boolValue] == TRUE) {
                  ...
                  if(arrayCount > 0) {
                      ...
                      [categoryList addObject:@"My Requests"];
...
- (void)DropDownListView:(DropDownListView *)dropdownListView didSelectedIndex:(NSInteger)anIndex{
    ...
    else if ([selectedCategory isEqualToString:@"My Requests"]) {
